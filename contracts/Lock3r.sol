...
import "./interfaces/Lock3r/ILock3rV1Helper.sol";
import "./libraries/Lock3rV1Library.sol";
import "./interfaces/IGovernance.sol";

contract Lock3rNetwork is ReentrancyGuard {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    /// @notice Lock3r Helper to set max prices for the ecosystem
    ILock3rV1Helper public LKRH;

    /// @notice EIP-20 token name for this token
    string public constant name = "Lock3r.Network";
    ...
    uint public totalSupply = 200000e18; // Initial 200000
    ...
    /// @notice Worked a job
    event LockerWorked(address indexed credit, address indexed job, address indexed locker, uint block);
    ...
    /// @notice 1 day to bond to become a Locker
     ...
     * @param amount the amount of credit being added to the job
     */
    function addLKRCredit(address job, uint amount) external {
        require(msg.sender == governance, "addLKRCredit: !gov");
        require(jobs[job], "addLKRCredit: !job");
    ...
    function worked(address Locker) external {
        workReceipt(Locker, LKRH.getQuoteLimit(_gasUsed.sub(gasleft())));
    }
    ...
    function workReceipt(address Locker, uint amount) public {
        ...
        require(amount <= LKRH.getQuoteLimit(_gasUsed.sub(gasleft())), "workReceipt: max limit");
        credits[msg.sender][address(this)] = credits[msg.sender][address(this)].sub(amount, "workReceipt: insuffient funds");
        lastJob[Locker] = now;
        _bond(address(this), Locker, amount);
        workCompleted[Locker] = workCompleted[Locker].add(amount);
        emit LockerWorked(address(this), msg.sender, Locker, block.number);
    }
    ...
    function receipt(address credit, address Locker, uint amount) external {
        ...
        emit LockerWorked(credit, msg.sender, Locker, block.number);
    }
    ...
    function receiptETH(address Locker, uint amount) external {
        ...
        emit LockerWorked(ETH, msg.sender, Locker, block.number);
    }
     ...
     * @param _lkrh new helper address to set
     */
    function setLock3rHelper(address _lkrh) external {
        require(msg.sender == governance, "setLock3rHelper: !gov");
        LKRH = ILock3rV1Helper(_lkrh);
    }
     ...
     */
    function isLocker(address Locker) external returns (bool) {
        _gasUsed = gasleft();
    ...
    function activate(address bonding) external {
        ...
        emit LockerBonded(msg.sender, block.number, block.timestamp, bonds[msg.sender][bonding]);
    }
    ...
    /**
     * @notice allows governance to create a dispute for a given Locker
     * @param Locker the address in dispute
     */
    function dispute(address Locker) external {
        require(msg.sender == governance, "dispute: !gov");
        disputes[Locker] = true;
        emit LockerDispute(Locker, block.number);
    }

    /**
     * @notice allows governance to slash a Locker based on a dispute
     * @param bonded the asset being slashed
     * @param Locker the address being slashed
     * @param amount the amount being slashed
     */
    function slash(address bonded, address Locker, uint amount) public nonReentrant {
        ...
        _unbond(bonded, Locker, amount);
        disputes[Locker] = false;
        emit LockerSlashed(Locker, msg.sender, block.number, amount);
    }
    ...
    function revoke(address Locker) external {
        ...
        slash(address(this), Locker, bonds[Locker][address(this)]);
    }

    /**
     * @notice allows governance to resolve a dispute on a Locker
     * @param Locker the address cleared
     */
    function resolve(address Locker) external {
        require(msg.sender == governance, "resolve: !gov");
        disputes[Locker] = false;
        emit LockerResolved(Locker, block.number);
    }
